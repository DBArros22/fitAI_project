...
function showView(viewId) {
    ...
    // 2. CORREÇÃO CRUCIAL: Varre e oculta absolutamente TODAS as telas do sistema para evitar vazamentos
    document.querySelectorAll('main[id^="view-"], section[id^="view-"], .page-container, [id^="view-"]').forEach(tela => {
        tela.classList.add('hidden');
        tela.style.display = 'none';
        tela.style.cssText = ''; // Limpa qualquer sujeira de estilo inline anterior
    });

    // 3. Localiza a view alvo com segurança expandida
    ...
    if (viewAlvo) {
        ...
        if (targetId === 'view-registro' || targetId === 'registro') {
            ...
            // Restaura rigorosamente o seu formato original de cards quadrados (3 colunas)
            viewAlvo.style.display = 'grid';
    ...
    // 5. Execução de hooks e funções vinculadas à tela ativa
...
function gerarCalendario() {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        calContainer.innerHTML += `
            <div onclick="alternarTreinoDia(${i})" style="cursor: pointer; display: flex; flex-direction: column; align-items: center; gap: 5px; user-select: none;">
            ...
            if (tipo === 'foto' || tipo === 'image') {
                midiaHtml = `
                    <div style="width: 100%; background: #090d16; border-radius: 12px; overflow: hidden; margin-top: 10px; display: flex; justify-content: center; align-items: center;">
                        <img src="${url}" style="width: 100%; height: auto; max-height: 420px; object-fit: contain; display: block; margin: 0 auto;" loading="lazy">
                    </div>
                `;
            } else if (tipo === 'video') {
                midiaHtml = `
                    <div style="width: 100%; background: #090d16; border-radius: 12px; overflow: hidden; margin-top: 10px;">
                        <video src="${url}" controls style="width: 100%; height: auto; max-height: 420px; object-fit: contain; display: block;" preload="none"></video>
                    </div>
                `;
            } else if (tipo === 'audio') {
...
function carregarNotificacoes() {
    ...
    // 1. Remove modal anterior instantaneamente se existir
    const modalAntigo = document.getElementById('modal-notificacoes-global');
    if (modalAntigo) modalAntigo.remove();

    // 2. Cria e exibe o esqueleto do modal de forma IMEDIATA (Síncrona)
    const modalEl = document.createElement('div');
    modalEl.id = 'modal-notificacoes-global';
    modalEl.style.cssText = "position: fixed !important; top: 50% !important; left: 50% !important; transform: translate(-50%, -50%) !important; width: 90%; max-width: 400px; max-height: 80vh !important; overflow-y: auto !important; background: #0b0f19; border: 1px solid rgba(255,255,255,0.15); border-radius: 16px; padding: 16px; z-index: 10000; box-shadow: 0 20px 40px rgba(0,0,0,0.6);";
    modalEl.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 8px;">
            <h3 style="color: white; font-size: 14px; margin: 0; font-weight: 800;">NOTIFICAÇÕES</h3>
            <button type="button" id="btn-fechar-notif" style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); color: #ef4444; width: 24px; height: 24px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 14px; cursor: pointer; font-weight: bold; line-height: 1;">&times;</button>
        </div>
    ...
    // Força a inserção imediata no DOM para que o usuário veja o modal abrir na mesma hora
    document.body.appendChild(modalEl);

    // 3. Vincula o evento de fechamento de forma síncrona e direta no botão recém-criado
    document.getElementById('btn-fechar-notif').addEventListener('click', (e) => {
        e.stopPropagation();
        modalEl.remove();
    });

    // 4. Busca os dados no Firebase em background sem travar a interface
    setTimeout(async () => {
        try {
            const snapshot = await db.collection('usuarios').doc(user.uid).collection('notificacoes').orderBy('criadoEm', 'desc').limit(15).get();
...
function clicarNotificacao(tipo, linkId, remetenteUid) {
    ...
    if (tipo === 'seguir' && remetenteUid) {
        if (typeof mudarAbaBlog === 'function') mudarAbaBlog('perfil', remetenteUid);
    } else if (linkId) {
        const postEl = document.getElementById(`post-feed-${linkId}`) || document.getElementById(`post-perfil-${linkId}`);
        if (postEl) {
            postEl.scrollIntoView({ behavior: 'smooth', block: 'center' });
            postEl.style.border = '1px solid #3b82f6';
            setTimeout(() => postEl.style.border = '1px solid rgba(255,255,255,0.08)', 2500);
        } else if (typeof carregarFeedDoBanco === 'function') {
            carregarFeedDoBanco().then(() => {
                setTimeout(() => {
                    const p = document.getElementById(`post-feed-${linkId}`);
                    if (p) {
                        p.scrollIntoView({ behavior: 'smooth', block: 'center' });
                        p.style.border = '1px solid #3b82f6';
                        setTimeout(() => p.style.border = '1px solid rgba(255,255,255,0.08)', 2500);
                    }
                }, 400);
            });
        }
...
function excluirPost(id) {
    ...
    modalConfirm.innerHTML = `
        <div class="glass-panel" style="max-width: 320px; width: 100%; padding: 25px; text-align: center; border: 1px solid #ef4444; background: #0f172a; border-radius: 24px; box-shadow: 0 0 40px rgba(239, 68, 68, 0.2);">
            ...
            <h3 style="color: white; margin-bottom: 8px; font-size: 1.05rem; letter-spacing: 1px; font-weight: 800;">EXCLUIR POST?</h3>
