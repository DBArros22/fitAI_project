    ...
    try {
        ...
        if (bio.trim() !== "") {
            blocoBioHtml = `
                <div style="display: flex; align-items: center; justify-content: center; gap: 8px; margin-bottom: 22px;">
                    <p id="texto-bio-usuario" style="color: #94a3b8; font-size: 13px; margin: 0; line-height: 1.5; max-width: 400px; word-break: break-word;">${bio}</p>
                    ${ehMeuPerfil ? `
                        <button onclick="ativarEdicaoBio()" style="background: rgba(255,255,255,0.08); border: 1px solid rgba(255,255,255,0.15); color: #3b82f6; border-radius: 8px; padding: 6px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.15)'" onmouseout="this.style.background='rgba(255,255,255,0.08)'" title="Editar Bio">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                        </button>
                    ` : ''}
                </div>
            ...
            if (ehMeuPerfil) {
                blocoBioHtml = `
                    <div style="margin-bottom: 22px;">
                        <button onclick="ativarEdicaoBio()" style="background: rgba(59,130,246,0.1); border: 1px dashed rgba(59,130,246,0.4); color: #3b82f6; padding: 10px 16px; border-radius: 12px; font-size: 12px; font-weight: 600; cursor: pointer; display: inline-flex; align-items: center; gap: 6px; transition: 0.2s;" onmouseover="this.style.background='rgba(59,130,246,0.2)'" onmouseout="this.style.background='rgba(59,130,246,0.1)'">
                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                            Adicionar bio ao perfil
                        </button>
                        <div id="texto-bio-usuario" style="display:none;"></div>
        ...
        container.innerHTML = `
            <div style="padding: 10px 0;">
                <div class="glass-panel" style="padding: 30px; border-radius: 28px; background: rgba(255,255,255,0.03); text-align: center; margin-bottom: 30px; border: 1px solid rgba(255,255,255,0.08); box-shadow: 0 10px 30px rgba(0,0,0,0.2);">
                    <div style="width: 90px; height: 90px; border-radius: 50%; background: #3b82f6; margin: 0 auto 15px; overflow: hidden; display: flex; align-items: center; justify-content: center; border: 3px solid #3b82f6; box-shadow: 0 0 20px rgba(59,130,246,0.4);">
                        ${foto ? `<img src="${foto}" style="width:100%; height:100%; object-fit:cover;">` : `<span style="color:white; font-size: 32px; font-weight:bold;">${nome.charAt(0)}</span>`}
                    </div>
                    <h3 style="color: white; font-size: 1.3rem; margin: 0 0 10px 0; font-weight: 800; letter-spacing: 1px;">${nome.toUpperCase()}</h3>
                    
                    ${blocoBioHtml}

                    <div style="display: flex; justify-content: center; gap: 30px; border-top: 1px solid rgba(255,255,255,0.08); padding-top: 18px;">
                        <div>
                            <p style="color: white; font-size: 18px; font-weight: bold; margin: 0;">${totalPosts}</p>
                            <p style="color: #64748b; font-size: 10px; margin: 0; text-transform: uppercase; letter-spacing: 1px;">Posts de Treino</p>
                        </div>
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(110px, 1fr)); gap: 12px;">
                    ${gridMidiasHtml || `<p style="color: #64748b; font-size: 12px; grid-column: 1 / -1; text-align: center; padding: 40px 0; background: rgba(255,255,255,0.01); border-radius: 14px; border: 1px solid rgba(255,255,255,0.04);">Nenhuma foto postada ainda.</p>`}
                </div>
            </div>
        `;
    } catch (e) {
